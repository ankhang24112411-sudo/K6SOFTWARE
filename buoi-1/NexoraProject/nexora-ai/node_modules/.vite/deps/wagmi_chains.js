import { A as fromNumber, B as trimLeft, D as from$6, Dt as encodeFunctionData, E as concat$1, Et as InvalidChainIdError$1, F as size, G as fromString$1, H as concat, I as slice$1, Jt as slice, K as isEqual, L as toBigInt, M as padLeft, P as random, R as toNumber, T as assert$3, U as from$7, V as validate, W as fromHex$1, _ as isAddressEqual, ct as InvalidLegacyVError, i as maxUint256, j as fromString, k as fromBytes, l as extract, m as TipAboveFeeCapError$1, n as assertRequest, nt as BaseError, o as defineTransactionRequest, p as FeeCapTooHighError$1, qt as concatHex, r as maxUint16, s as formatTransactionRequest$1, tt as stringify, u as getCallError, ut as InvalidStorageKeySizeError$1, w as InvalidLengthError, z as toString } from "./call-DjzTRLjq.js";
import { C as toBytes$1, E as numberToHex$1, F as trim, H as InvalidAddressError, N as hexToNumber$1, O as toHex$1, R as pad, T as bytesToHex, U as BaseError$1, W as size$1, _ as isAddress, j as hexToBigInt, m as parseAccount, x as hexToBytes, y as keccak256 } from "./createBatchScheduler-BfmwkS-o.js";
import { $ as toBlobSidecars, A as validate$1, D as from$9, E as checksum, F as keccak256$1, G as formatLog, H as defineTransactionReceipt, I as sha256$1, L as validate$2, M as fromHex$3, N as toBytes$2, O as fromPublicKey, P as toHex$2, Q as getTransactionType, S as yParityToV, T as assert$5, U as formatTransactionReceipt$1, V as getCode, _ as fromRpc$5, at as blobsToProofs, b as toRpc$5, d as fromRpcList$1, et as EmptyBlobError, f as toRpcList$1, ft as defineBlock, g as fromHex$2, h as from$10, ht as formatTransaction$1, i as AccountNotFoundError, it as sha256, j as from$8, k as isEqual$1, m as extract$1, mt as defineTransaction, n as sendTransaction, nt as InvalidVersionedHashVersionError, o as verifyHash$1, ot as blobsToCommitments, p as assert$4, rt as commitmentsToVersionedHashes, t as writeContract, tt as InvalidVersionedHashSizeError, u as getSelector, v as fromTuple$3, vt as toRlp, w as create, x as toTuple$3, xt as getAction, y as toHex$3, yt as readContract, z as parseEventLogs } from "./writeContract-CEJCl1Tq.js";
import { a as sha256$2, i as Field, o as sha384, r as createCurve, s as sha512, t as secp256k1 } from "./secp256k1-uPGXaB0V.js";
...
function recoverPublicKey(options) {
	...
	const point = new secp256k1.Signature(BigInt(r), BigInt(s)).addRecoveryBit(yParity).recoverPublicKey(from$6(payload).substring(2));
	return from$8(point);
}
/**
...
function verify$4(options) {
	...
	if (options.address) return isEqual$1(options.address, recoverAddress({
		payload,
		signature: options.signature
	}));
	return secp256k1.verify(options.signature, from$7(payload), toBytes$2(options.publicKey), ...hash ? [{
		prehash: true,
		lowS: true
	}] : []);
}
...
function verify$2(options) {
	...
	if (rpId !== void 0) {
		...
		const expectedRpIdHash = sha256$1(fromString(rpId), { as: "Bytes" });
	...
	const clientDataJSONHash = sha256$1(fromString$1(clientDataJSON), { as: "Bytes" });
	return verify$3({
		hash: true,
...
function assert$2(config) {
	...
	if (owners.length > 255) throw new InvalidConfigError({ reason: "too many owners" });
	...
	if (Number(threshold) > 8) throw new InvalidConfigError({ reason: "threshold exceeds max threshold" });
	let totalWeight = 0;
	let previous;
	for (const owner of owners) {
		if (!validate$1(owner.owner) || toBigInt(owner.owner) === 0n) throw new InvalidConfigError({ reason: "owner cannot be zero" });
...
function getAddress(config) {
	assert$2(config);
	const hash = keccak256$1(concat$1(fromString(accountDomain), padLeft(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 1 }), fromNumber(config.owners.length, { size: 1 }), ...config.owners.flatMap((owner) => [owner.owner, fromNumber(owner.weight, { size: 1 })])));
	const account = from$9(slice$1(hash, 12, 32));
...
function getSignPayload$2(value) {
	...
	return keccak256$1(concat$1(fromString(signatureDomain), from$6(payload), account));
}
/**
...
function assertMultisig(envelope, depth) {
	...
	if (!validate$1(envelope.account)) throw new InvalidMultisigApprovalError({ reason: "multisig account is invalid" });
	...
	if (envelope.init) {
		assert$2(envelope.init);
		if (!isEqual$1(getAddress(envelope.init), envelope.account)) throw new InvalidMultisigApprovalError({ reason: "multisig init does not derive account" });
	}
	for (const inner of envelope.signatures) {
		...
		if (type === "multisig") {
			...
			if (multisig.init) throw new InvalidMultisigApprovalError({ reason: "nested multisig owner approvals cannot carry `init`" });
			assertMultisig(multisig, depth + 1);
		} else assert$1(inner);
		if (size(serialize$2(inner)) > 2049) throw new InvalidMultisigApprovalError({ reason: `multisig owner signature exceeds ${maxOwnerSignatureBytes} bytes` });
	}
}
/**
...
function deserialize_(value, multisigDepth) {
	const serialized = value.endsWith("0x7777777777777777777777777777777777777777777777777777777777777777".slice(2)) ? slice$1(value, 0, -size(magicBytes)) : value;
	...
	const typeId = slice$1(serialized, 0, 1);
	const data = slice$1(serialized, 1);
	...
	if (typeId === serializedP256Type) {
		...
		return {
			publicKey: {
				prefix: 4,
				x: toBigInt(slice$1(data, 64, 96)),
				y: toBigInt(slice$1(data, 96, 128))
			},
			prehash: toNumber(slice$1(data, 128, 129)) !== 0,
			signature: {
				r: toBigInt(slice$1(data, 0, 32)),
				s: toBigInt(slice$1(data, 32, 64))
			},
			type: "p256"
		};
	}
	if (typeId === serializedWebAuthnType) {
		...
		const webauthnData = slice$1(data, 0, webauthnDataSize);
		let authenticatorData;
		let clientDataJSON;
		for (let split = 37; split < webauthnDataSize; split++) {
			const potentialJson = toString(slice$1(webauthnData, split));
			if (potentialJson.startsWith("{") && potentialJson.endsWith("}")) try {
				JSON.parse(potentialJson);
				authenticatorData = slice$1(webauthnData, 0, split);
		...
		return {
			publicKey: {
				prefix: 4,
				x: toBigInt(slice$1(data, webauthnDataSize + 64, webauthnDataSize + 96)),
				y: toBigInt(slice$1(data, webauthnDataSize + 96, webauthnDataSize + 128))
			},
			metadata: {
				authenticatorData,
				clientDataJSON
			...
			signature: {
				r: toBigInt(slice$1(data, webauthnDataSize, webauthnDataSize + 32)),
				s: toBigInt(slice$1(data, webauthnDataSize + 32, webauthnDataSize + 64))
			},
			type: "webAuthn"
		};
	}
	if (typeId === serializedKeychainType || typeId === serializedKeychainV2Type) return {
		userAddress: slice$1(data, 0, 20),
		inner: deserialize_(slice$1(data, 20), multisigDepth),
		type: "keychain",
	...
	if (typeId === serializedMultisigType) {
		...
		if (!Array.isArray(decoded) || decoded.length !== 2) throw new InvalidSerializedError$1({
		...
		for (const signature of signatures) if (size(signature) > 2049) throw new InvalidSerializedError$1({
			reason: `multisig owner signature exceeds ${maxOwnerSignatureBytes} bytes`,
			serialized
		});
		if (!Array.isArray(address) && !validate$1(address)) throw new InvalidSerializedError$1({
			reason: "invalid multisig account",
			serialized
		});
		if (Array.isArray(address)) {
			const [salt, threshold, owners] = address;
			if (address.length !== 3 || Array.isArray(salt) || size(salt) !== 32 || Array.isArray(threshold) || size(threshold) > 1 || !Array.isArray(owners) || owners.some((owner) => !Array.isArray(owner) || owner.length !== 2 || owner.some(Array.isArray) || size(owner[1]) > 1)) throw new InvalidSerializedError$1({
				reason: "invalid multisig init config",
				serialized
			});
		}
		const init = Array.isArray(address) ? fromTuple$2(address) : void 0;
		const envelope = {
			type: "multisig",
			account: init ? getAddress(init) : address,
...
function fromRpc$4(envelope) {
	...
	if (envelope.type === "multisig" || "signatures" in envelope && ("account" in envelope || "init" in envelope)) {
		...
		if (hasAccount === hasInit) throw new InvalidMultisigApprovalError({ reason: "RPC multisig must contain exactly one of `account` or `init`" });
		const init = hasInit ? from$4(multisig.init) : void 0;
		const result = {
			type: "multisig",
			account: init ? getAddress(init) : multisig.account,
			signatures: multisig.signatures.map((signature) => fromRpc$4(signature)),
			...init ? { init } : {}
		};
		assert$1(result);
		return result;
...
function getType$1(envelope) {
	...
	if (("account" in envelope || "genesisConfig" in envelope || "init" in envelope) && "signatures" in envelope) return "multisig";
	throw new CoercionError({ envelope });
}
/**
...
function serialize$2(envelope, options = {}) {
	...
	if (type === "multisig") {
		const multisig = envelope;
		assert$1(multisig);
		return concat$1(serializedMultisigType, fromHex([multisig.init ? toTuple$2(multisig.init) : multisig.account, multisig.signatures.map((signature) => serialize$2(signature))]), options.magic ? magicBytes : "0x");
	}
	throw new CoercionError({ envelope });
}
/**
...
function toRpc$4(envelope) {
	...
	if (type === "multisig") {
		const multisig = envelope;
		assert$1(multisig);
		const signatures = multisig.signatures.map((signature) => toRpc$4(signature));
		if (multisig.init) return {
			init: {
				...multisig.init,
...
function verify(signature, parameters) {
	...
	if (envelope.type === "p256") {
		...
		if (!isEqual$1(envelopeAddress, address)) return false;
		return verify$3({
...
function from$1(authorization, options = {}) {
	...
	if (options.signature) {
		const signature = from$3(options.signature);
...
function fromTuple(tuple) {
	...
	if (signatureSerialized) {
		const signature = deserialize$1(signatureSerialized);
		assertSignature(signature);
...
function toTuple(authorization) {
	...
	const signature = (() => {
		...
		return serialize$2(authorization.signature);
	})();
	const type = (() => {
...
function assertSignature(signature) {
	if (signature.type === "keychain" || signature.type === "multisig") throw new InvalidSignatureTypeError(signature.type);
}
...
function deserialize(serialized) {
	const transactionArray = toHex(slice$1(serialized, 1));
	...
	if (feePayerSignatureOrSender !== "0x" && feePayerSignatureOrSender !== void 0) {
		if (feePayerSignatureOrSender === "0x00" || validate$1(feePayerSignatureOrSender)) {
			transaction.feePayerSignature = null;
			if (validate$1(feePayerSignatureOrSender)) transaction.from = feePayerSignatureOrSender;
...
function from(envelope, options = {}) {
	...
	return {
		...
		...feePayerSignature ? { feePayerSignature: from$10(feePayerSignature) } : {},
		type: "tempo"
	};
}
/**
...
async function serializeTempo(transaction, sig) {
	...
	const signature = (() => {
		...
		if (!transaction.multisig || !transaction.signatures) return void 0;
		const sorted = sortMultisigApprovals({
			payload: getSignPayload(from(transaction_sender_ox)),
	...
	if (signature && typeof transaction.feePayer === "object") {
		...
		return serialize$1(tx, { feePayerSignature: from$10(feePayerSignature) });
	}
...
var chainConfig = {
	...
	prepareTransactionRequest: [async (r, { client, phase }) => {
		...
		if (multisig) {
			const config = from$4(multisig);
			request.multisig = config;
			request.from = getAddress(config);
			request.multisigInit = {
				salt: config.salt ?? zeroSalt,
